...
## 20220715 G.Caglio - actually not supported
...
##$month_start=date('2022-04-01');
##$month_end=date('2022-04-17',strtotime("-1 day"));

##$monthb4_start=date('2022-03-01',strtotime("-1 month"));
##$monthb4_end=date('2022-03-17',strtotime("-1 month",strtotime($month_end)));
...
$sql="select project_name,truncate(sum(split_daily_costs),2) as Cost from ".$db_csvcost_table." where fake_date>='".$monthb4_start."' and fake_date<='".$monthb4_end."' group by project_name order by project_name;";
echo "DEBUG : costs for previous month\n";
echo $sql."\n\n";
$result = mysqli_query($link, $sql);
if (mysqli_num_rows($result) > 0) { 
  //for each row insert in the associative array the label as the key, and the costs as column[0]
  while($row = mysqli_fetch_assoc($result)) {	  
    $resgrp=$row["project_name"];
    $cost=$row["Cost"];
...
# and store them in the associative array.
#AZ $sql="select ResourceGroup,truncate(sum(SplittedCost),2) as Cost from ".$db_splittedcosts_table." where Date>='".$month_start."' and Date<='".$month_end."' group by ResourceGroup order by ResourceGroup; ";
$sql="select project_name,truncate(sum(split_daily_costs),2) as Cost from ".$db_csvcost_table." where fake_date>='".$month_start."' and fake_date<='".$month_end."' group by project_name order by project_name;";
echo "DEBUG : costs for current month\n";
echo $sql."\n\n";
$result = mysqli_query($link, $sql);
if (mysqli_num_rows($result) > 0) {
...
# print html output
echo "<html><head><title>Projects month-to-day report</title></head><body>\r\n";
echo "<table width='90%' cellspacing='0' cellpadding='0'>\r\n";
echo "<tr><th>Project</th><th>Prev. month-to-date<br /><small>[".$monthb4_start."-".$monthb4_end."]</small></th><th>Current month-to-date<br /><small>[".$month_start."-".$month_end."]</small></th><th>Delta cost</th><th>Percentage</th><th>Trend</th></tr>\r\n";
